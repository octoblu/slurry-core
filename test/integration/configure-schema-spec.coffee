...
describe 'configure schema', ->
  beforeEach (done) ->
    ...
    @configureHandler = configureSchema: sinon.stub()
  ...
  describe 'On GET /v1/configure-schema', ->
    describe 'when the configure-handler yields an empty object', ->
      beforeEach (done) ->
        @configureHandler.configureSchema.yields null, {}

        options =
          baseUrl: "http://localhost:#{@serverPort}"
          json: true

        request.get '/v1/configure-schema', options, (error, @response, @body) =>
          done error
    ...
    describe 'when the configure-handler yields a larger schema', ->
      beforeEach (done) ->
        @configureHandler.configureSchema.yields null, {
          doSomething:
            type: 'object'
    ...
    describe 'when the configure-handler yields an error', ->
      beforeEach (done) ->
        error = new Error 'Something is awry'
        error.code = 418
        @configureHandler.configureSchema.yields error
